...
```{r, echo=FALSE}
repo <- PaRe::Repository$new("../")
```

## Package overview
```{r packageDiagram, out.width="100%", warning=FALSE}
repo %>%
  PaRe::pkgDiagram() %>%
...
```{r getDefinedFunctionsPkg, message=FALSE, warning=FALSE}
...
### Summary
```{r lintScores, message=FALSE, warning=FALSE}
...
### Warnings
```{r lintMessages warnings, message=FALSE, warning=FALSE}
DT::datatable(
  lintMsgs %>%
...
### Style
```{r lintMessages style, message=FALSE, warning=FALSE}
DT::datatable(
  lintMsgs %>%
...
```{r checkDependencies, warning=FALSE}
...
```{r dependencyGraphStats, message=FALSE, warning=FALSE}
...
```{r plotFunctionUse, dpi=100, fig.height=25, out.width="100%", message=FALSE, warning=FALSE}
